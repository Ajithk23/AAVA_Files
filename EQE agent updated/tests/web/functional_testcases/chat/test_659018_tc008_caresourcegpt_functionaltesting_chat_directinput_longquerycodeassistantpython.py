"""TC-INT-08 — Verify Chat Threads are created when initiating long query with Code Assist - Python using 5-mini INT model via Direct input.

Test Case Name : TC08 - Verify Chat Threads are created when initiating long query with
                 Code Assist - Python using 5-mini INT model via Direct input
Description    : Verify chat creation - long query using direct input in "How can I help?"
                 text box in Code Assist - Python.
...
from utils.test_data_loader import get_chat_int_tc08_inputs as _get_inputs, ChatIntTC08Inputs
...
_TC_NAME = "TC-INT-08"
_TC_INT_08_FILE_PREFIX = "TC-INT-08"
...
async def _run_tc_flow(
    ...
    td: ChatIntTC08Inputs,
) -> None:
    """Execute all 8 steps for TC-INT-08.

    Flow summary:
      Step  1    : Navigate to INT URL.
      Step  2    : Verify landing page options (Chat, File Management, Prompt Library).
      Step  3    : Navigate to Chat, click + New Chat, configure 5-mini INT + Code Assistant - Python.
    ...
    try:
        logger.info("[TC-INT-08] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-08] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        3,
        f"Navigate to the Chat module and click on + New Chat button to initiate a chat. Go to Chat Configurations, Select Model {td.model_name} and Chat Type as Code Assistant - Python.",
...
async def test_tc_int_08_verify_chat_threads_long_query_code_assist_python_5mini_int_direct_input(
    docuchat_context,
):
    """TC-INT-08 — Verify Chat Threads are created when initiating long query with Code Assist - Python using 5-mini INT model via Direct input."""
    td = _get_inputs()
