"""TC-INT-04 — Verify Chat Threads are created when initiating long query with Code Assist - Python using 5-mini INT model via Saved Prompts.

Test Case Name : TC04 - Verify Chat Threads are created when initiating long query with
                 Code Assist - Python using 5-mini INT model via Saved Prompts
Description    : Verify chat creation - long query using saved prompt in Code Assist - Python.
...
from utils.test_data_loader import get_chat_int_tc04_inputs as _get_inputs, ChatIntTC04Inputs
...
_TC_NAME = "TC-INT-04"
_TC_INT_04_FILE_PREFIX = "TC-INT-04"
...
async def _run_tc_flow(
    ...
    td: ChatIntTC04Inputs,
) -> None:
    """Execute all 13 steps for TC-INT-04.

    Flow summary:
      Steps 1-2  : Navigate to INT URL, verify landing page.
      Steps 3-6  : Create a saved prompt (long query) in Prompt Library.
      Step  7    : Navigate to Chat, configure 5-mini INT + Code Assist - Python.
    ...
    async def _step_7_navigate_chat_and_configure() -> None:
        ...
        await _select_model_and_chat_type(
            page, model_name=td.model_name, chat_type=td.chat_type
        )
    ...
    try:
        logger.info("[TC-INT-04] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-04] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        7,
        f"Navigate to Chat module, click + New Chat, select Model {td.model_name} and Chat Type as Code Assist - Python",
...
async def test_tc_int_04_verify_chat_threads_long_query_code_assist_python_5mini_int_saved_prompts(
    docuchat_context,
):
    """TC-INT-04 — Verify Chat Threads are created when initiating long query with Code Assist - Python using 5-mini INT model via Saved Prompts."""
    td = _get_inputs()
