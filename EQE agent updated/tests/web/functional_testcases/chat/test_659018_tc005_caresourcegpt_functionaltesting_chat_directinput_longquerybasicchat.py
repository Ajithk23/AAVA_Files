"""TC-INT-05 — Verify Chat Threads are created when initiating long query with Basic Chat using 5-mini INT model via Direct input.

Test Case Name : TC05 - Verify Chat Threads are created when initiating long query with
...
from utils.test_data_loader import get_chat_int_tc05_inputs as _get_inputs, ChatIntTC05Inputs
...
_TC_NAME = "TC-INT-05"
_TC_INT_05_FILE_PREFIX = "TC-INT-05"
...
async def _run_tc_flow(
    ...
    td: ChatIntTC05Inputs,
) -> None:
    """Execute all 8 steps for TC-INT-05.
    ...
    try:
        logger.info("[TC-INT-05] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-05] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        4,
        "Manually input a long query  \"How can I help?\" text box and verify that the full prompt content is automatically displayed in the \"How can I help?\" text box and click on the Send button to initiate chat.",
...
async def test_tc_int_05_verify_chat_threads_long_query_basic_chat_5mini_int_direct_input(
    docuchat_context,
):
    """TC-INT-05 — Verify Chat Threads are created when initiating long query with Basic Chat using 5-mini INT model via Direct input."""
    td = _get_inputs()
