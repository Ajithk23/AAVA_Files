"""TC-INT-06 — Verify Chat Threads are created when initiating long query with Chat with Documents using 5-mini INT model via Direct input.

Test Case Name : TC06 - Verify Chat Threads are created when initiating long query with
...
from utils.test_data_loader import get_chat_int_tc06_inputs as _get_inputs, ChatIntTC06Inputs
...
_TC_NAME = "TC-INT-06"
_TC_INT_06_FILE_PREFIX = "TC-INT-06"
...
async def _type_long_query_and_send(page: Page, long_query: str) -> None:
    """Step 5: Manually input the long query into the 'How can I help?' text box and click Send.
...
async def _run_tc_flow(
    ...
    td: ChatIntTC06Inputs,
) -> None:
    """Execute all 11 steps for TC-INT-06.
    ...
    Flow summary:
      ...
      Step  6    : Manually type the long query into 'How can I help?' text box and click Send.
      ...
      Step  10   : Cleanup — navigate to File Management and delete the uploaded document.
      Step  11   : Exit.
    """
    ...
    async def _step_3_navigate_chat_and_configure() -> None:
        ...
        # _open_chat_workspace clicks the Chat radio/tab and waits for the workspace
        # to be ready (model combobox / textarea visible).
    ...
    try:
        logger.info("[TC-INT-06] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-06] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        4,
        f"Navigate to the Chat module and click on + New Chat button to initiate a chat. Go to Chat Configurations, Select Model {td.model_name} and Chat Type as Chat with Documents.",
...
async def test_tc_int_06_verify_chat_threads_long_query_chat_with_documents_5mini_int_direct_input(
    docuchat_context,
):
    """TC-INT-06 — Verify Chat Threads are created when initiating long query with Chat with Documents using 5-mini INT model via Direct input."""
    td = _get_inputs()
