"""TC-INT-03 — Verify Chat Threads are created when initiating long query with Document Review using 5-mini INT model via Saved Prompts.

Test Case Name : TC03 - Verify Chat Threads are created when initiating long query with
                 Document Review using 5-mini INT model via Saved Prompts
Description    : Verify chat creation - long query using saved prompt in Document Review.
...
from utils.test_data_loader import get_chat_int_tc03_inputs as _get_inputs, ChatIntTC03Inputs
...
_TC_NAME = "TC-INT-03"
_TC_INT_03_FILE_PREFIX = "TC-INT-03"
...
async def _run_tc_flow(
    ...
    td: ChatIntTC03Inputs,
) -> None:
    """Execute all 13 steps for TC-INT-03.

    Flow summary:
      Steps 1-2  : Navigate to INT URL, verify landing page.
      Steps 3-6  : Create a saved prompt (long query) in Prompt Library.
      Steps 7    : Navigate to Chat, configure 5-mini INT + Document Review.
    ...
    try:
        logger.info("[TC-INT-03] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-03] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        7,
        f"Navigate to Chat module, click + New Chat, select Model {td.model_name} and Chat Type as Document Review",
...
async def test_tc_int_03_verify_chat_threads_long_query_document_review_5mini_int_saved_prompts(
    docuchat_context,
):
    """TC-INT-03 — Verify Chat Threads are created when initiating long query with Document Review using 5-mini INT model via Saved Prompts."""
    td = _get_inputs()
