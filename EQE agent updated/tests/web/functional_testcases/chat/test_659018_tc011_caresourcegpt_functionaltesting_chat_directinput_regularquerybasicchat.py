"""TC-INT-11 — Verify Chat Threads are created when initiating regular query with Basic Chat using 5-mini INT model via Direct input.

Test Case Name : TC011 - Verify Chat Threads are created when initiating regular query with
                 Basic Chat  using 5-mini INT model via Direct input
Description    : Verify chat creation - regular query using direct input in "How can I help?"
                 text box in Basic Chat.
...
from utils.test_data_loader import get_chat_int_tc11_inputs as _get_inputs, ChatIntTC11Inputs
...
async def _type_regular_query_and_send(page: Page, query: str) -> None:
    """Step 4: Manually input the regular query into the 'How can I help?' text box and click Send.
...
async def _run_tc_flow(
    ...
    td: ChatIntTC11Inputs,
) -> None:
    """Execute all 8 steps for TC-INT-11.

    Flow summary:
      Step  1    : Navigate to INT URL.
      Step  2    : Verify landing page options (Chat, File Management, Prompt Library).
      Step  3    : Navigate to Chat, click + New Chat, configure 5-mini INT + Basic Chat.
      Step  4    : Manually type the regular query into 'How can I help?' text box and click Send.
    ...
    async def _step_3_navigate_chat_and_configure() -> None:
        ...
        # _open_chat_workspace may no-op when already on the Chat page
        # (matches the existing textarea) without actually starting a new chat.
        new_chat_candidates = [
    ...
    try:
        logger.info("[TC-INT-11] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-11] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        4,
        "Manually input a regular query  \"How can I help?\" text box and verify that the full prompt content is automatically displayed in the \"How can I help?\" text box and click on the Send button to initiate chat. ",
...
async def test_tc_int_11_verify_chat_threads_regular_query_basic_chat_5mini_int_direct_input(
    docuchat_context,
):
    """TC-INT-11 — Verify Chat Threads are created when initiating regular query with Basic Chat using 5-mini INT model via Direct input."""
    td = _get_inputs()
