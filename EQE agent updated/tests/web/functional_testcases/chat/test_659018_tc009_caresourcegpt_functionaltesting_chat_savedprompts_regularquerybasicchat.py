"""TC-INT-09 — Verify Chat Threads are created when initiating regular query with Basic Chat type using 5-mini INT model via Saved Prompts.

Test Case Name : TC09 - Verify Chat Threads are created when initiating regular query with
                 Basic Chat type using 5-mini INT model via Saved Prompts
Description    : Verify chat creation - regular query using saved prompt in Basic Chat.
...
from utils.test_data_loader import get_chat_int_tc09_inputs as _get_inputs, ChatIntTC09Inputs
...
async def _wait_for_chat_response(page: Page, chat_page: ChatPage) -> None:
    ...
    assert latest_message is not None and latest_message.strip(), (
        "System did not generate a response after sending the regular query via Saved Prompts"
    )
...
async def _verify_chat_thread_in_sidebar(page: Page) -> None:
    ...
    assert thread_found, (
        "No chat thread was created in the sidebar after sending the regular query via Saved Prompts. "
...
async def _run_tc_flow(
    ...
    td: ChatIntTC09Inputs,
) -> None:
    """Execute all 13 steps for TC-INT-09.

    Flow summary:
      Steps 1-2  : Navigate to INT URL, verify landing page.
      Steps 3-6  : Create a saved prompt (regular query) in Prompt Library.
    ...
    try:
        logger.info("[TC-INT-09] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-09] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        5,
        "Click inside the Title (optional) input field and enter a descriptive name. Click inside the Description text area and input the required query.",
    ...
    await _run_step(
        7,
        f"Navigate to the Chat module and click on + New Chat button to initiate a chat.\nGo to Chat Configurations, Select Model {td.model_name} and Chat Type as Basic Chat.",
    ...
    await _run_step(
        8,
        "Navigate to Saved Prompts and select the required prompt (regular prompt). Verify that the full prompt content is automatically displayed in the \"How can I help?\" text box and click on the Send button to initiate chat.",
...
async def test_tc_int_09_verify_chat_threads_regular_query_basic_chat_5mini_int_saved_prompts(
    docuchat_context,
):
    """TC-INT-09 — Verify Chat Threads are created when initiating regular query with Basic Chat type using 5-mini INT model via Saved Prompts."""
    td = _get_inputs()
