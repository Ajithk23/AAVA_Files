"""TC-INT-10 — Verify Chat Threads are created when initiating regular query with Chat with Documents using 5-mini INT model via Saved Prompts.

Test Case Name : TC10 - Verify Chat Threads are created when initiating regular query with
                 Chat with Documents using 5-mini INT model via Saved Prompts
Description    : Verify that a chat thread is created when a regular query is initiated via a
...
from utils.test_data_loader import get_chat_int_tc10_inputs as _get_inputs, ChatIntTC10Inputs
...
_TC_NAME = "TC-INT-10"
_TC_INT_10_FILE_PREFIX = "TC-INT-10"
...
async def _wait_for_chat_response(page: Page, chat_page: ChatPage) -> None:
    ...
    latest_message = ""
    deadline = time.monotonic() + 120  # 120s ceiling — regular queries should respond promptly
    while time.monotonic() < deadline:
...
async def _run_tc_flow(
    ...
    td: ChatIntTC10Inputs,
) -> None:
    """Execute all 16 steps for TC-INT-10.

    Flow summary:
      Steps 1-2  : Navigate to INT URL, verify landing page.
      Step  3    : Navigate to File Management and upload the test document.
      Steps 4-7  : Create a saved prompt (regular query) in Prompt Library.
      ...
      Step  15   : Cleanup — navigate to File Management and delete the uploaded document.
      Step  16   : Exit.
    """
    page: Page = docuchat_context["page"]
    settings: dict = docuchat_context["settings"]
    _file_name = td.file_paths[0].name
    ...
    try:
        logger.info("[TC-INT-10] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-10] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    await _run_step(
        3,
        f"Navigate to File Management, upload {_file_name} and verify it appears in the file table.",
        _navigate_to_file_management(page),
        _TC_NAME, page,
    )
    await _run_step(
        3,
        f"Upload {_file_name} via Browse Files and verify it is listed in the uploaded files table.",
    ...
    await _run_step(
        10,
        "Navigate to Saved Prompts, select the regular prompt, verify full prompt content auto-fills the 'How can I help?' text box, click Send",
...
async def test_tc_int_10_verify_chat_threads_regular_query_chat_with_documents_5mini_int_saved_prompts(
    docuchat_context,
):
    """TC-INT-10 — Verify Chat Threads are created when initiating regular query with Chat with Documents using 5-mini INT model via Saved Prompts."""
    td = _get_inputs()
