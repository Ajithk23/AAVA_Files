"""TC-INT-01 — Verify Chat Threads are created when initiating long query with Basic Chat type using 5-mini INT model via Saved Prompts.

Test Case Name : TC01 - Verify Chat Threads are created when initiating long query with
                 Basic Chat type using 5-mini INT model via Saved Prompts
Description    : Verify chat creation - long query using saved prompt in Basic Chat.
...
from utils.test_data_loader import get_chat_int_tc01_inputs as _get_inputs, ChatIntTC01Inputs
...
_TC_NAME = "TC-INT-01"
_TC_INT_01_FILE_PREFIX = "TC-INT-01"
...
async def _select_saved_prompt_and_send(
    ...
    """Step 8: Select the named saved prompt, verify text box auto-fills, click Send.
    ...
    # Wait longer for the baseweb dropdown to fully render all options
...
async def _wait_for_chat_response(page: Page, chat_page: ChatPage) -> None:
    """Step 9: Wait (up to 120s) for the AI response to appear, then wait for it to fully complete."""
    latest_message = ""
...
async def _verify_chat_thread_in_sidebar(page: Page) -> None:
    """Step 10: Verify a chat thread is created and visible in the sidebar."""
...
async def _run_tc_flow(
    ...
    td: ChatIntTC01Inputs,
) -> None:
    """Execute all 13 steps for TC-INT-01.

    Flow summary:
      Steps 1-2  : Navigate to INT URL, verify landing page.
      Steps 3-6  : Create a saved prompt (long query) in Prompt Library.
      Step  7    : Navigate to Chat, configure 5-mini INT + Basic Chat.
      Steps 8-10 : Use the saved prompt to send the query; verify response and thread creation.
      Step  11   : Cleanup — delete the chat thread created during this test.
      Step  12   : Cleanup — navigate to Prompt Library and delete the created prompt.
      Step  13   : Exit.
    """
    ...
    async def _step_7_navigate_chat_and_configure() -> None:
        ...
        await _select_model_and_chat_type(
            page, model_name=td.model_name, chat_type="Basic Chat"
        )
    ...
    try:
        logger.info("[TC-INT-01] Waiting up to 4 minutes for Okta sign-in and Streamlit landing page to load...")
        await page.wait_for_function(_STREAMLIT_READY_JS, timeout=240000)
        logger.info("[TC-INT-01] Streamlit landing page ready — URL: %s", page.url)
    except Exception:
    ...
    # Step 3 also requires clicking Create New Prompt after navigating
    await _run_step(
        3,
    ...
    await _run_step(
        5,
        "Click inside the Title (optional) input field and enter a descriptive name. Click inside the Description text area and input the required long query.",
    ...
    await _run_step(
        6,
        "Locate the Display Order field and enter a numeric value to define ordering. Review all entered values and click 'Create Prompt'. Verify that the newly created prompt is visible.",
    ...
    await _run_step(
        7,
        f"Navigate to the Chat module and click on + New Chat button to initiate a chat. Go to Chat Configurations, Select Model {td.model_name} and Chat Type as Basic Chat.",
    ...
    await _run_step(
        8,
        "Navigate to Saved Prompts and select the required prompt (long prompt). Verify that the full prompt content is automatically displayed in the 'How can I help?' text box and click on the Send button to initiate chat.",
        _step_8_select_saved_prompt_and_send(),
        _TC_NAME, page,
    )
    ...
    await _run_step(
        10,
        "Observe the chat interface for thread creation — verify the chat thread is created and visible in the user's thread list.",
...
async def test_tc_int_01_verify_chat_threads_long_query_basic_chat_5mini_int_saved_prompts(
    docuchat_context,
):
    """TC-INT-01 — Verify Chat Threads are created when initiating long query with Basic Chat type using 5-mini INT model via Saved Prompts."""
    td = _get_inputs()
