"""TC034 Regression — DocuChat File Management: Validate Chat Initialization with 4.1-mini CRT model for all the supported file types used by the client.

Test Case Name : TC034_Regression_DocuChat_File Management_Validate Chat Initialization with 4.1-mini CRT model for all the supported file types used by the client
...
from utils.test_data_loader import get_tc034_inputs as _get_tc034_inputs
...
_TC034_CHAT_FILE = "test_upload.pdf"
...
async def _run_tc_flow(
    ...
    """Execute all 11 steps for TC034.
    ...
    6.  Navigate to Chat > Chat Configuration, select Model 4.1-mini CRT and Chat Type as
        Chat with Documents.
    ...
    await _run_step(
        7,
        f"Click 'Choose Options' dropdown and verify '{_TC034_CHAT_FILE}' is visible",
        _verify_ready_files_in_chat_dropdown(page, [_TC034_CHAT_FILE]),
        tc_name, page,
    )
    await _run_step(
        8,
        f"Select '{_TC034_CHAT_FILE}' from the 'Choose Options' dropdown",
        _select_all_files_in_chat_dropdown(page, [_TC034_CHAT_FILE]),
        tc_name, page,
    )
    await _run_step(
        9,
...
async def test_tc034_regression_docuchat_file_management_validate_chat_initialization_with_41mini_crt_model_for_all_the_supported_file_types_used_by_the_client(
    docuchat_context,
):
    """TC034 — Validate Chat Initialization with 4.1-mini CRT model for all the supported file types used by the client."""
    try:
        await _run_tc_flow(docuchat_context, tc_name="TC034")
    finally:
