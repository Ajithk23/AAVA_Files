"""TC032 Regression — CareSourceGPT File Management: Validate Chat Initialization with 5-mini CRT model for all the supported file types used by the client.

Test Case Name : TC032_Regression_CareSourceGPT_File Management_Validate Chat Initialization with 5-mini CRT model for all the supported file types used by the client
...
from utils.test_data_loader import get_tc032_inputs as _get_tc032_inputs
...
_TC032_CHAT_FILE = "test_upload.pdf"
...
async def _run_tc_flow(
    ...
    """Execute all 11 steps for TC032.
    ...
    6.  Navigate to Chat > Chat Configuration, select Model 5-mini CRT and Chat Type as
        Chat with Documents.
    7.  Click "Choose Options" dropdown and verify all uploaded files are visible.
    8.  Select all uploaded files from the dropdown.
    ...
    _td = _get_tc032_inputs()

    file_names = [Path(fp).name for fp in _td.file_paths]
    ...
    async def _step_6_navigate_and_configure_chat() -> None:
        # Navigate from File Management back to the Chat section first,
        # then open the chat workspace and configure model + chat type.
    ...
    await _run_step(
        7,
        f"Click 'Choose Options' dropdown and verify '{_TC032_CHAT_FILE}' is visible",
        _verify_ready_files_in_chat_dropdown(page, [_TC032_CHAT_FILE]),
        tc_name, page,
    )
    await _run_step(
        8,
        f"Select '{_TC032_CHAT_FILE}' from the 'Choose Options' dropdown",
        _select_all_files_in_chat_dropdown(page, [_TC032_CHAT_FILE]),
        tc_name, page,
    )
    await _run_step(
        9,
...
async def test_tc032_regression_caresourcegpt_file_management_validate_chat_initialization_with_5mini_crt_model_for_all_the_supported_file_types_used_by_the_client(
    docuchat_context,
):
    """TC032 — Validate Chat Initialization with 5-mini CRT model for all the supported file types used by the client."""
    try:
        await _run_tc_flow(docuchat_context, tc_name="TC032")
    finally:
