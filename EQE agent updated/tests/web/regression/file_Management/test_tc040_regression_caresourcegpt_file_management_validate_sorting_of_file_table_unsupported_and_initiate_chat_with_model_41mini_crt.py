"""TC040 Regression - CareSourceGPT File Management: Validate sorting of file table,
unsupported/duplicate/huge file upload, cancel upload, upload progress, delete file,
download processed documents and initiate chat with model 4.1-mini CRT."""
...
async def _run_tc_flow(docuchat_context: dict) -> None:
    ...
    await _run_step(7, "Verify File Name column sorted correctly",
                    _verify_sort_column(page, "File Name"), _TC_NAME, page)
    await _run_step(8, "Verify File Status column sorted correctly",
                    _verify_sort_column(page, "Status"), _TC_NAME, page)
    await _run_step(9, "Verify File Size column sorted correctly",
                    _verify_sort_column(page, "File Size"), _TC_NAME, page)
    await _run_step(10, "Verify File Type column sorted correctly",
                    _verify_sort_column(page, "File Type"), _TC_NAME, page)
    await _run_step(11, "Verify Uploaded Date column sorted correctly",
                    _verify_sort_column(page, "Uploaded Date"), _TC_NAME, page)
    await _run_step(12, "Verify ID column sorted correctly",
                    _verify_sort_column(page, "ID"), _TC_NAME, page)

    # Step 13
    await _run_step(13,
                    "Verify default sort restored after navigating away and back",
                    _verify_sort_after_navigation(page), _TC_NAME, page)

    # Steps 14-15
    await _run_step(14,
                    "Verify Unsupported File Type: select .exe/.zip/.epub file via Browse/Drag",
                    _try_unsupported_file_upload(page, file_page), _TC_NAME, page)
    await _run_step(15,
                    "Click Upload for unsupported file; verify system restricts and shows error",
                    _try_unsupported_file_upload(page, file_page), _TC_NAME, page)
    ...
    async def _wrap_huge_upload() -> None:
        huge_file_name_holder[0] = await _upload_huge_file(page, file_page)

    await _run_step(16,
                    "Verify Huge File Upload (>30 MB): verify progress bar and file visible in list",
                    _wrap_huge_upload(), _TC_NAME, page)

    # Step 17
    await _run_step(17,
                    "Verify Duplicate File Upload: upload same file; verify duplicate handled",
                    _upload_duplicate_file(page, file_page, huge_file_name_holder[0]), _TC_NAME, page)

    # Steps 18-20
    await _run_step(18, "Verify Cancel Upload: select large file via Browse / Drag and Drop",
                    _verify_cancel_upload(page, file_page), _TC_NAME, page)
    await _run_step(19, "Click Upload; verify upload starts with progress indicator",
                    _verify_cancel_upload(page, file_page), _TC_NAME, page)
    await _run_step(20, "Click Cancel while uploading; verify upload stops immediately",
                    _verify_cancel_upload(page, file_page), _TC_NAME, page)

    # Steps 21-25
    await _run_step(21, "Verify File Upload Progress: select file via Browse Files / Drag and Drop",
                    _verify_upload_progress(page, file_page), _TC_NAME, page)
    await _run_step(22, "Click Upload; verify upload starts with progress indicator",
                    _verify_upload_progress(page, file_page), _TC_NAME, page)
    await _run_step(23, "Observe upload progress indicator during file upload",
                    _verify_upload_progress(page, file_page), _TC_NAME, page)
    await _run_step(24, "Verify Refresh button; ensure file upload successful",
                    _verify_upload_progress(page, file_page), _TC_NAME, page)
    await _run_step(25, "Verify progress reaches 100% and file appears in file list",
                    _verify_upload_progress(page, file_page), _TC_NAME, page)

    # Step 26
    await _run_step(26, "Upload multiple valid documents of different file types",
                    _upload_multiple_valid_documents(page, file_page), _TC_NAME, page)

    # Steps 27-28
    await _run_step(27,
                    "Select single file; verify Delete Document and Download Processed Document buttons",
                    _verify_delete_single_file(page, _TC_NAME), _TC_NAME, page)
    await _run_step(28, "Click Delete Document; verify confirmation message",
                    _verify_delete_single_file(page, _TC_NAME), _TC_NAME, page)

    # Steps 29-30
    await _run_step(29,
                    "Select multiple documents; verify Delete Documents and Download Processed Documents",
                    _verify_delete_multiple_files(page, _TC_NAME), _TC_NAME, page)
    await _run_step(30, "Click Delete Documents; verify confirmation message",
                    _verify_delete_multiple_files(page, _TC_NAME), _TC_NAME, page)

    # Step 31
    await _run_step(31, "Upload multiple valid documents for download verification",
                    _upload_multiple_valid_documents(page, file_page), _TC_NAME, page)
    ...
    await _run_step(33, "Click Download Processed Document; verify .txt file downloaded",
                    _verify_download_single_file(page, _TC_NAME), _TC_NAME, page)

    # Steps 34-35
    await _run_step(34,
                    "Select multiple files; verify Download Processed Documents button",
                    _verify_download_multiple_files(page, _TC_NAME), _TC_NAME, page)
    await _run_step(35,
                    "Click Download Processed Documents; verify .zip file downloaded",
                    _verify_download_multiple_files(page, _TC_NAME), _TC_NAME, page)
    ...
    await _run_step(36,
                    "Navigate to Chat > Chat Configurations, select Model 4.1-mini CRT and Chat Type as "
                    "Chat with Documents; verify Choose Options dropdown visible",
                    _step_36(), _TC_NAME, page)

    # Step 37
    await _run_step(37, "Click Choose Options dropdown and verify uploaded files list is visible",
                    _open_choose_options_dropdown(page), _TC_NAME, page)

    # Step 38
    await _run_step(38,
                    "Select the huge file and duplicate from dropdown; verify selected and highlighted",
...
async def test_tc040_regression_caresourcegpt_file_management_validate_sorting_of_file_table_unsupported_and_initiate_chat_with_model_41mini_crt(
    docuchat_context,
):
    """TC040 - Validate sorting, unsupported/duplicate/huge file upload, delete, download, chat with 4.1-mini CRT."""
    try:
