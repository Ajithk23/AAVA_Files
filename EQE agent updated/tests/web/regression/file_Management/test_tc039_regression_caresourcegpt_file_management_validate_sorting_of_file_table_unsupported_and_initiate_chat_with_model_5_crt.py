"""TC039 Regression - CareSourceGPT File Management: Validate sorting of file table,
unsupported/duplicate/huge file upload, cancel upload, upload progress, delete file,
download processed documents and initiate chat with model 5 CRT."""
...
async def _run_tc_flow(docuchat_context: dict) -> None:
    """Execute all 41 steps for TC039 (includes pre-test cleanup)."""
    page = docuchat_context["page"]
    settings = docuchat_context["settings"]
    _td = _get_inputs("tc039")
    ...
    await _run_step(7, "Verify File Name column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "File Name"), _TC_NAME, page)
    await _run_step(8, "Verify File Status column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "Status"), _TC_NAME, page)
    await _run_step(9, "Verify File Size column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "File Size"), _TC_NAME, page)
    await _run_step(10, "Verify File Type column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "File Type"), _TC_NAME, page)
    await _run_step(11, "Verify Uploaded Date column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "Uploaded Date"), _TC_NAME, page)
    await _run_step(12, "Verify ID column sorted correctly (ascending and descending)",
                    _verify_sort_column(page, "ID"), _TC_NAME, page)

    # Step 13
    await _run_step(13,
                    "Verify default sort restored after navigating away and back to File Management",
    ...
    await _run_step(15,
                    "Click Upload for unsupported file; verify system restricts upload and shows error",
                    _try_unsupported_file_upload(page, file_page), _TC_NAME, page)
    ...
    # Step 17
    await _run_step(17,
                    "Verify Duplicate File Upload: upload same file again; verify duplicate handled",
    ...
    # Steps 29-30
    await _run_step(29,
                    "Select multiple documents; verify Delete Documents and Download Processed Documents buttons",
...
async def test_tc039_regression_caresourcegpt_file_management_validate_sorting_of_file_table_unsupported_and_initiate_chat_with_model_5_crt(
    docuchat_context,
):
    """TC039 - Validate sorting, unsupported/duplicate/huge file upload, delete, download, chat with 5 CRT."""
    try:
