"""TC004 Regression — DocuChat Chat: Validate Chat Initialization with 5-mini CRT model and Chat Type as Chat with Documents.

Test Case Name : TC004_Regression_DocuChat_Chat_Validate Chat Initialization with 5-mini CRT model and Chat Type as Chat with Documents
...
# TC004 identifier prefix — used to select the matching pre-uploaded document in File Management.
# The QA environment has a file pre-uploaded whose name includes this prefix.
_TC004_FILE_PREFIX = "TC004_Regression_DocuChat_Chat_Validate Chat Initialization with 5-mini CRT model and Chat Type as Chat with Documents"
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC004 (Chat with Documents, 5-mini CRT model).
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc004"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc004_regression_docuchat_chat_validate_chat_initialization_with_5mini_crt_model_and_chat_with_documents(
    docuchat_context,
):
    """TC004 — Validate Chat Initialization with 5-mini CRT model and Chat Type as Chat with Documents."""
    _td = _get_chat_inputs("tc004")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC004",
        model_name=_td.model_name,
        file_prefix=_TC004_FILE_PREFIX,
    )
