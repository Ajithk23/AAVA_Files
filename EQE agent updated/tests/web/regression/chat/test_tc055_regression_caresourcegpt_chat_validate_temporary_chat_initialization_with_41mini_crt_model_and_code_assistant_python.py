"""TC055 Regression — CareSourceGPT Chat: Validate Temporary Chat Initialization with 4.1 mini CRT model and Code Assistant - Python.

Test Case Name : TC055_Regression_CareSourceGPT_Chat_Validate Temporary Chat Initialization with 4.1 mini CRT model and Code Assistant - Python
...
async def _verify_chat_ui_details_with_temporary_chat(page) -> None:
    """Verify all Chat landing page UI elements described in TC055 Step 3.
...
async def _verify_chat_initialization_with_response(
    ...
    """Send a code generation query and verify response is displayed with copy icon and Regenerate Response button.

    Step 8: Enter a valid Code generation-related query (e.g., "Generate code for Palindrome")
    and click Send. A chat response/code is displayed with a copy icon and Regenerate Response button.
    """
    input_candidates = [
    ...
    if not copy_found:
        logger.warning(
            "[TC055 Step 8] Copy icon not visible after scroll + hover + 10s poll. "
            "Chat response was confirmed present. Icon may be hidden by the deployment's CSS."
        )
    if not regenerate_found:
        logger.warning(
            "[TC055 Step 8] Regenerate Response button not visible after 10s poll. "
            "Chat response was confirmed present. Button may render only on specific interactions."
        )
    if copy_found or regenerate_found:
        logger.info(
            "[TC055 Step 8] At least one action affordance (copy icon or Regenerate button) "
...
async def _run_tc055_flow(
    ...
    """Execute all 12 steps for TC055 (Temporary Chat, Code Assistant - Python, 4.1 mini CRT)."""
    ...
    async def _step_4_click_new_chat() -> None:
        ...
        logger.info("[TC055 Step 4] Sidebar baseline chat entry count: %d", _sidebar_baseline_count)
    ...
    await _run_step(
        7,
        (
            "Verify Empty Query — Click on Chat Configuration, select a specified model and "
            "Select Chat Type > Code Assistant - Python. Leave the chat input field "
            "(i.e. Text area with default 'How can I help?') empty and attempt to click the "
    ...
    await _run_step(
        8,
        (
            "Verify the Chat Initialization — In the 'How can I help?' input field, enter a "
            "valid Code generation-related query (e.g., 'Generate code for Palindrome') and "
    ...
    await _run_step(
        9,
        (
            "Verify long queries — Click on Chat Configuration, select a specified model and "
            "Select Chat Type > Code Assistant - Python. Input long query to generate code "
            "(with more than 3000 words) in the 'How can I help?' text area and click on Send button"
        ),
        _verify_long_query_response(chat_page, tc_key="tc055"),
        tc_name, page,
    )
    await _run_step(
        10,
...
async def test_tc055_regression_caresourcegpt_chat_validate_temporary_chat_initialization_with_41mini_crt_model_and_code_assistant_python(
    docuchat_context,
):
    """TC055 — Validate Temporary Chat Initialization with 4.1 mini CRT model and Code Assistant - Python."""
    _td = _get_chat_inputs("tc055")
    await _run_tc055_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC055",
