"""TC003 Regression — DocuChat Chat: Validate Chat Initialization with 4.1 mini CRT model and Basic Chat type.

Test Case Name : TC003_Regression_DocuChat_Chat_Validate Chat Initialization with 4.1 mini CRT model and Basic Chat type
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC003 (Basic Chat, 4.1 mini CRT model).

    Steps mirror TC001/TC002 exactly — only the model name differs.
    """
    ...
    await _run_step(
        4,
        f"Go to Chat Configuration, select Model {model_name} and Chat Type as Basic Chat",
    ...
    await _run_step(
        6,
        "Verify chat initialization with valid query",
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc003"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc003_regression_docuchat_chat_validate_chat_initialization_with_41mini_crt_model_and_basic_chat_type(
    docuchat_context,
):
    """TC003 — Validate Chat Initialization with 4.1 mini CRT model and Basic Chat type."""
    _td = _get_chat_inputs("tc003")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC003",
