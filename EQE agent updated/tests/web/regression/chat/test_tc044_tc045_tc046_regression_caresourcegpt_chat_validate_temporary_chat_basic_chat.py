"""TC044 / TC045 / TC046 Regression — CareSourceGPT Chat:
Validate Temporary Chat Initialization with Basic Chat type.

Parametrized over three model variants:
  TC044 — 5-mini CRT model
  TC045 — 5 CRT model
  TC046 — 4.1 mini CRT model
...
async def _run_tc_flow(
    ...
    """Execute all 12 steps for Temporary Chat Initialization with Basic Chat type.

    Key differences from TC001-TC003 (regular chat):
    ...
    - Step 6     : Select model and Basic Chat type in Chat Configuration.
    - Step 8     : Submit valid query and verify response with copy + regenerate button.
    - Step 9     : Submit long query (>3000 words) and verify response without errors.
    - Step 10    : Verify NO chat history is created for the temporary session.
    - Step 11    : Click '+ New Chat' and verify previous temp session is not shown.
    ...
    async def _step_6_configure_chat() -> None:
        ...
        # Log active values after selection — DOM snapshot before proceeding
    ...
    await _run_step(
        6,
        f"Go to Chat Configuration, select Model = {model_name} and Chat Type = Basic Chat",
    ...
    await _run_step(
        8,
        "Verify chat initialization — enter a valid CareSource-related query "
        f"(e.g. '{query}') and click Send; verify response with copy icon and "
    ...
    await _run_step(
        9,
        "Verify long query (>3000 words) — response generated without latency or errors",
...
async def test_temporary_chat_initialization_basic_chat(
    docuchat_context,
    model_name: str,
    tc_name: str,
    tc_key: str,
):
    """Validate Temporary Chat Initialization with Basic Chat type across all model variants.

    Parametrize IDs: TC044 (5-mini CRT), TC045 (5 CRT), TC046 (4.1 mini CRT).
    To run a single variant: pytest -k TC044
