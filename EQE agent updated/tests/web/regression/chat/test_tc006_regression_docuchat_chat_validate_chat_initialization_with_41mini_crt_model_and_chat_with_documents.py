"""TC006 Regression — DocuChat Chat: Validate Chat Initialization with 4.1-mini CRT model and Chat Type as Chat with Documents.

Test Case Name : TC006_Regression_DocuChat_Chat_Validate Chat Initialization with 4.1 mini CRT model and Chat Type as Chat with Documents
...
# TC006 identifier prefix — used to select the matching pre-uploaded document in File Management.
_TC006_FILE_PREFIX = "TC006_Regression_DocuChat_Chat_Validate Chat Initialization with 4.1 mini CRT model and Chat Type as Chat with Documents"
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC006 (Chat with Documents, 4.1-mini CRT model).
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc006"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc006_regression_docuchat_chat_validate_chat_initialization_with_41mini_crt_model_and_chat_with_documents(
    docuchat_context,
):
    """TC006 — Validate Chat Initialization with 4.1-mini CRT model and Chat Type as Chat with Documents."""
    _td = _get_chat_inputs("tc006")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC006",
        model_name=_td.model_name,
        file_prefix=_TC006_FILE_PREFIX,
    )
