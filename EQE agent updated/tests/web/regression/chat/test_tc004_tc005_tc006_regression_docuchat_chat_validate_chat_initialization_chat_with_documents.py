"""TC004 / TC005 / TC006 Regression — DocuChat Chat: Validate Chat Initialization with Chat Type as Chat with Documents.

TC004 — 5-mini CRT model
TC005 — 5 CRT model
TC006 — 4.1 mini CRT model
...
# ── Model variants ────────────────────────────────────────────────────────────
# Each param: (model_name, tc_key, file_prefix)
# file_prefix must match the pre-uploaded document name prefix in File Management.
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for Chat Initialization with Chat with Documents chat type.

    Key differences vs Basic Chat (TC001-TC003):
    - Step 4 : After selecting the model + chat type, selects a document from the
...
@pytest.mark.parametrize("model_name, tc_key, file_prefix", _MODEL_VARIANTS)
async def test_chat_initialization_chat_with_documents(
    docuchat_context,
    model_name: str,
    tc_key: str,
    file_prefix: str,
):
    """Validate Chat Initialization with Chat Type as Chat with Documents for each model variant.

    Parametrize IDs: TC004 (5-mini CRT), TC005 (5 CRT), TC006 (4.1 mini CRT).
    To run a single variant: pytest -k TC004
