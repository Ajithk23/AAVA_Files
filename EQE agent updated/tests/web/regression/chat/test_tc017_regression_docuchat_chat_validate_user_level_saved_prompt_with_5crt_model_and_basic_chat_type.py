"""TC017 Regression — DocuChat Chat: Validate user level saved prompt with 5 CRT model and Basic Chat type.

Test Case Name : TC017_Regression_DocuChat_Chat_Validate user level saved prompt with
                 5 CRT model and Basic Chat type
...
Pre-condition  : User has valid DocuChat access. Application should have Prompts created
                 (specific to Chat type selected and support long queries of 3000 words) in
                 Prompt Library and can be selected in Chat module under Saved Prompts section.
Feature        : Chat
Environment    : QA-CRT
"""
...
_TC017_NAME = (
    "TC017_Regression_DocuChat_Chat_Validate user level saved prompt with "
    "5 CRT model and Basic Chat type"
)
...
async def _run_tc_flow(
    ...
    """Execute all 11 steps for TC017.
...
async def test_tc017_regression_docuchat_chat_validate_user_level_saved_prompt_with_5crt_model_and_basic_chat_type(
    docuchat_context,
):
    """TC017 — Validate user level saved prompt with 5 CRT model and Basic Chat type."""
    _td = _get_chat_inputs("tc017")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name=_TC017_NAME,
