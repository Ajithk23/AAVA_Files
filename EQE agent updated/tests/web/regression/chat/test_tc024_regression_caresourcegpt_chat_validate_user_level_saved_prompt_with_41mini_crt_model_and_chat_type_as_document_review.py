"""TC024 Regression — CareSourceGPT Chat: Validate user level saved prompt with 4.1 mini CRT model and Chat type as Document Review.

Test Case Name : TC024_Regression_CareSourceGPT_Chat_Validate user level saved prompt with 4.1 mini CRT model and Chat type as Document Review
...
_TC024_EDITED_CHAT_TITLE = "Edited Chat Title"
...
async def _verify_chat_ui_details_caresourcegpt(page) -> None:
    """Verify all Chat landing page UI elements described in TC024 Step 3."""
    checks = [
        (
            "New Chat button",
            [
...
async def _send_current_input_and_verify_response(
    ...
    assert latest_message and latest_message.strip(), (
        "Chat response was not generated after sending the saved prompt"
    )
...
async def _verify_long_query_with_document_review_saved_prompt(
    ...
    """Step 7: Select Document Review, select a saved prompt with >3000 words, send and verify.

    Strategy:
    1. Open Chat Configuration and select Chat Type as Document Review.
    ...
    if selected_text and len(selected_text.split()) >= 100:
        ...
        await _verify_long_query_response(chat_page, tc_key="tc024")
...
async def _initiate_chat_with_updated_chat(page, chat_page: ChatPage) -> None:
    ...
    Searches the sidebar for a chat whose title contains _TC024_EDITED_CHAT_TITLE.
    ...
    for index in range(button_count):
        try:
            ...
            if _TC024_EDITED_CHAT_TITLE.lower() in text.lower():
...
async def _run_tc024_flow(
    ...
    """Execute all 11 steps for TC024 (Document Review, 4.1 mini CRT, Saved Prompts)."""
    ...
    await _run_step(
        6,
        "Verify the Chat Initialization — in the 'How can I help?' input field the saved prompt details (with special characters/grammatical errors/semantical errors) are visible, click Send and verify chat response is displayed with proper response, copy icon and Regenerate Response button",
        _send_current_input_and_verify_response(page, chat_page, fallback_query=query),
        tc_name, page,
    )
    await _run_step(
        7,
        "Verify long queries — go to Chat Configuration, select Document Review, select a saved prompt with more than 3000 words, input long query (with more than 3000 words with grammatical/semantical errors) in the 'How can I help?' text area and click Send, verify response generated without latency or errors",
...
async def test_tc024_regression_caresourcegpt_chat_validate_user_level_saved_prompt_with_41mini_crt_model_and_chat_type_as_document_review(
    docuchat_context,
):
    """TC024 — Validate user level saved prompt with 4.1 mini CRT model and Chat type as Document Review."""
    _td = _get_chat_inputs("tc024")
    await _run_tc024_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC024",
