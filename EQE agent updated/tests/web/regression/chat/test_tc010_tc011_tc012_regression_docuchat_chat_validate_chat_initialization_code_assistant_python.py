"""TC010 / TC011 / TC012 Regression — DocuChat Chat: Validate Chat Initialization with Code Assistant - Python.

Parametrized over three model variants:
  TC010 — 5-mini CRT model
  TC011 — 5 CRT model
  TC012 — 4.1 mini CRT model
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for Code Assistant - Python Initialization (TC010 / TC011 / TC012).

    The only difference between variants is the model name selected in Step 4.
    Step 6 uses _verify_code_assistant_response (validates code block + copy/regenerate options).
    """
...
async def test_chat_initialization_code_assistant_python(
    docuchat_context,
    model_name: str,
    tc_name: str,
    tc_key: str,
):
    """Validate Chat Initialization with Code Assistant - Python across all model variants.

    Parametrize IDs: TC010 (5-mini CRT), TC011 (5 CRT), TC012 (4.1 mini CRT).
    To run a single variant: pytest -k TC010
