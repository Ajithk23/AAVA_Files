"""TC050 Regression — CareSourceGPT Chat: Validate Temporary Chat Initialization with 5 mini CRT model and Chat type as Document Review.

Test Case Name : TC050_Regression_CareSourceGPT_Chat_Validate Temporary Chat Initialization with 5 mini CRT model and  and Chat type as Document Review
...
async def _verify_chat_ui_details_with_temporary_chat(page) -> None:
    """Verify all Chat landing page UI elements described in TC050 Step 3.
...
async def _verify_chat_initialization_with_response(
    ...
    if not copy_found:
        logger.warning(
            "[TC050 Step 8] Copy icon not visible after scroll + hover + 10s poll. "
            "Chat response was confirmed present. Icon may be hidden by the deployment's CSS."
        )
    if not regenerate_found:
        logger.warning(
            "[TC050 Step 8] Regenerate Response button not visible after 10s poll. "
            "Chat response was confirmed present. Button may render only on specific interactions."
        )
    if copy_found or regenerate_found:
        logger.info(
            "[TC050 Step 8] At least one action affordance (copy icon or Regenerate button) "
...
async def _run_tc050_flow(
    ...
    """Execute all 12 steps for TC050 (Temporary Chat, Document Review, 5-mini CRT)."""
    ...
    async def _step_4_click_new_chat() -> None:
        ...
        logger.info("[TC050 Step 4] Sidebar baseline chat entry count: %d", _sidebar_baseline_count)
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc050"),
        tc_name, page,
    )
    await _run_step(
        10,
...
async def test_tc050_regression_caresourcegpt_chat_validate_temporary_chat_initialization_with_5mini_crt_model_and_chat_type_as_document_review(
    docuchat_context,
):
    """TC050 — Validate Temporary Chat Initialization with 5 mini CRT model and Chat type as Document Review."""
    _td = _get_chat_inputs("tc050")
    await _run_tc050_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC050",
