"""TC008 Regression — DocuChat Chat: Validate Chat Initialization with 5 CRT model and Document Review.

Test Case Name : TC008_Regression_DocuChat_Chat_Validate Chat Initialization with 5 CRT model and Document Review
...
# TC008 identifier prefix — used to select the matching pre-uploaded document in File Management.
_TC008_FILE_PREFIX = "TC008_Regression_DocuChat_Chat_Validate Chat Initialization with 5 CRT model and Document Review"
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC008 (Document Review, 5 CRT model).
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc008"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc008_regression_docuchat_chat_validate_chat_initialization_with_5crt_model_and_document_review(
    docuchat_context,
):
    """TC008 — Validate Chat Initialization with 5 CRT model and Document Review."""
    _td = _get_chat_inputs("tc008")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC008",
        model_name=_td.model_name,
        file_prefix=_TC008_FILE_PREFIX,
    )
