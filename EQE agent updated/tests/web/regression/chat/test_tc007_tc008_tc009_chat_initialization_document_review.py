"""TC007 / TC008 / TC009 Regression — DocuChat Chat: Validate Chat Initialization with Document Review chat type.

Parametrized over three model variants:
  TC007 — 5-mini CRT model  (with pre-uploaded file selection)
  TC008 — 5 CRT model       (with pre-uploaded file selection)
  TC009 — 4.1 mini CRT model (no file selection required per test specification)
...
# Each tuple: (model_name, tc_name, tc_key, file_prefix)
# file_prefix is None for TC009 — that TC does not require pre-uploaded file selection.
_MODEL_VARIANTS = [
    pytest.param(
        "5-mini CRT",
        "TC007",
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for Document Review Initialization (TC007 / TC008 / TC009).

    TC007 and TC008 select a pre-uploaded document in Step 4.
    TC009 does not require file selection per test specification (file_prefix=None).
    Step 6 uses _verify_document_review_response (validates copy icon + regenerate button).
    Query is intentionally garbled/slang text to stress Document Review grammar correction.
    """
    ...
    await _run_step(
        4,
        f"Go to Chat Configuration, select Model {model_name}, Chat Type as Document Review"
        + (", and select at least one document" if file_prefix is not None else ""),
...
@pytest.mark.parametrize("model_name, tc_name, tc_key, file_prefix", _MODEL_VARIANTS)
async def test_chat_initialization_document_review(
    docuchat_context,
    model_name: str,
    tc_name: str,
    tc_key: str,
    ...
    """Validate Chat Initialization with Document Review chat type across all model variants."""
