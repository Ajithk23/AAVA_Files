"""TC022 Regression — CareSourceGPT Chat: Validate user level saved prompt with 5-mini CRT model and Chat type as Document Review.

Test Case Name : TC022_Regression_CareSourceGPT_Chat_Validate user level saved prompt with 5-mini CRT model and Chat type as Document Review
...
_TC022_EDITED_CHAT_TITLE = "Edited Chat Title"
...
async def _verify_chat_ui_details_caresourcegpt(page) -> None:
    """Verify all Chat landing page UI elements described in TC022 Step 3."""
    checks = [
        (
            "New Chat button",
            [
...
async def _verify_long_query_with_document_review_saved_prompt(
    ...
    if selected_text and len(selected_text.split()) >= 100:
        ...
        await _verify_long_query_response(chat_page, tc_key="tc022")
...
async def _initiate_chat_with_updated_chat(page, chat_page: ChatPage) -> None:
    ...
    Searches the sidebar for a chat whose title contains _TC022_EDITED_CHAT_TITLE.
    ...
    for index in range(button_count):
        try:
            ...
            if _TC022_EDITED_CHAT_TITLE.lower() in text.lower():
...
async def _run_tc022_flow(
    ...
    """Execute all 11 steps for TC022 (Document Review, 5-mini CRT, Saved Prompts)."""
...
async def test_tc022_regression_caresourcegpt_chat_validate_user_level_saved_prompt_with_5mini_crt_model_and_chat_type_as_document_review(
    docuchat_context,
):
    """TC022 — Validate user level saved prompt with 5-mini CRT model and Chat type as Document Review."""
    _td = _get_chat_inputs("tc022")
    await _run_tc022_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC022",
