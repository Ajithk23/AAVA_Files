"""TC021 Regression — DocuChat Chat: Validate user level saved prompt with 4.1 mini CRT model and Chat Type as Chat with Documents.

Test Case Name : TC021_Regression_DocuChat_Chat_Validate user level saved prompt with
                 4.1 mini CRT model and Chat Type as Chat with Documents
...
_TC021_NAME = (
    "TC021_Regression_DocuChat_Chat_Validate user level saved prompt with "
    "4.1 mini CRT model and Chat Type as Chat with Documents"
)
...
async def _run_tc_flow(
    ...
    """Execute all 11 steps for TC021.
    ...
    Step 4  : Go to Chat Configurations, select Model 4.1-mini CRT and Chat Type as
    ...
    async def _step_4_configure_model_and_chat_type() -> None:
        ...
        await _select_file_for_chat_context(page, tc_name_prefix=_TC021_FILE_PREFIX)
    ...
    async def _step_7_verify_long_query() -> None:
        await _verify_long_query_with_saved_prompt_chat_with_documents(
            page, chat_page, model_name, tc_key, file_prefix=_TC021_FILE_PREFIX
        )
...
async def test_tc021_regression_docuchat_chat_validate_user_level_saved_prompt_with_41mini_crt_model_and_chat_with_documents(
    docuchat_context,
):
    """TC021 — Validate user level saved prompt with 4.1 mini CRT model and Chat Type as Chat with Documents."""
    _td = _get_chat_inputs("tc021")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name=_TC021_NAME,
