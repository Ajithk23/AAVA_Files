"""TC001 / TC002 / TC003 Regression — DocuChat Chat: Validate Chat Initialization with Basic Chat type.

Parametrized over three model variants:
  TC001 — 5-mini CRT model
  TC002 — 5 CRT model
  TC003 — 4.1 mini CRT model
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for Basic Chat Initialization (TC001 / TC002 / TC003).
    ...
    await _run_step(
        6,
        "Verify chat initialization with valid query — response displayed with copy and regenerate options",
        _step_6_initialize_chat(),
        tc_name, page,
    )
    await _run_step(
        7,
        "Verify long query behavior (>3000 words) — response generated without errors",
...
async def test_chat_initialization_basic_chat(
    docuchat_context,
    model_name: str,
    tc_name: str,
    tc_key: str,
):
    """Validate Chat Initialization with Basic Chat type across all model variants.

    Parametrize IDs: TC001 (5-mini CRT), TC002 (5 CRT), TC003 (4.1 mini CRT).
    To run a single variant: pytest -k TC001
