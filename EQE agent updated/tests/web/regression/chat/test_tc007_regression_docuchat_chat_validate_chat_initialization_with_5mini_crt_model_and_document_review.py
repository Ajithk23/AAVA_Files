"""TC007 Regression — DocuChat Chat: Validate Chat Initialization with 5-mini CRT model and Document Review.

Test Case Name : TC007_Regression_DocuChat_Chat_Validate Chat Initialization with 5-mini CRT model and Document Review
Description    : Verify DocuChat initializes Chat successfully with 'Document Review' chat type,
                 user can start interaction, prevents empty query submission, handles very long
                 queries gracefully without any error, handles chat Edit/Delete functionality
                 from the Chat selected in Chat history without any errors.
...
# TC007 identifier prefix — used to select the matching pre-uploaded document in File Management.
_TC007_FILE_PREFIX = "TC007_Regression_DocuChat_Chat_Validate Chat Initialization with 5-mini CRT model and Document Review"
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC007 (Document Review, 5-mini CRT model).

    Key differences vs Basic Chat (TC001-TC003):
    - Step 4 : After selecting model + chat type 'Document Review', selects a document from the
               'Select Ready Files to Include in Chat Context' multiselect.
    - Step 6 : Uses _verify_document_review_response (validates copy icon + regenerate button).
    - Query  : Intentionally garbled/slang text to stress Document Review grammar correction.
    """
    ...
    await _run_step(
        4,
        f"Go to Chat Configuration, select Model {model_name}, Chat Type as Document Review, and select at least one document",
    ...
    await _run_step(
        6,
        "Verify Document Review chat initialization with intentionally garbled query — response displayed with copy and regenerate options",
        _step_6_initialize_chat(),
        tc_name, page,
    )
    await _run_step(
        7,
        "Verify long query behavior (>3000 words) — response without errors",
        _verify_long_query_response(chat_page, tc_key="tc007"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc007_regression_docuchat_chat_validate_chat_initialization_with_5mini_crt_model_and_document_review(
    docuchat_context,
):
    """TC007 — Validate Chat Initialization with 5-mini CRT model and Document Review."""
    _td = _get_chat_inputs("tc007")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC007",
        model_name=_td.model_name,
        file_prefix=_TC007_FILE_PREFIX,
    )
