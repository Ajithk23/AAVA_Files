"""TC012 Regression — DocuChat Chat: Validate Chat Initialization with 4.1 mini CRT model and Code Assistant - Python.

Test Case Name : TC012_Regression_DocuChat_Chat_Validate Chat Initialization with 4.1 mini CRT model and Code Assistant - Python
...
from tests.web.regression.chat.chat_test_helpers import (
    ...
    _verify_code_assistant_response,
...
async def _run_tc_flow(
    ...
    """Execute all 14 steps for TC012 (Code Assistant - Python, 4.1 mini CRT model)."""
    ...
    async def _step_4_configure_chat() -> None:
        ...
        await _select_model_and_chat_type(page, model_name=model_name, chat_type="Code Assistant - Python")

    # ── Step 6 helpers ───────────────────────────────────────────────────────
    async def _step_6_initialize_chat() -> None:
        await _verify_send_button_enables_for_valid_input(page)
        await _verify_code_assistant_response(chat_page, query)
    ...
    await _run_step(
        4,
        f"Go to Chat Configuration, select Model {model_name} and Chat Type as Code Assistant - Python",
    ...
    await _run_step(
        6,
        "Verify Code Assistant - Python chat initialization with code generation query — response/code displayed with copy and regenerate options",
        _step_6_initialize_chat(),
        tc_name, page,
    )
    await _run_step(
        7,
        "Verify long query behavior (>3000 words code generation query) — response/code generated without latency or errors",
        _verify_long_query_response(chat_page, tc_key="tc012"),
        tc_name, page,
    )
    await _run_step(
        8,
...
async def test_tc012_regression_docuchat_chat_validate_chat_initialization_with_41mini_crt_model_and_code_assistant_python(
    docuchat_context,
):
    """TC012 — Validate Chat Initialization with 4.1 mini CRT model and Code Assistant - Python."""
    _td = _get_chat_inputs("tc012")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC012",
