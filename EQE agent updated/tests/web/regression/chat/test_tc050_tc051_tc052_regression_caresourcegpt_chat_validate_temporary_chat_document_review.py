"""TC050 / TC051 / TC052 Regression — CareSourceGPT Chat:
Validate Temporary Chat Initialization with Chat Type as Document Review.

Parametrized over three model variants:
  TC050 — 5-mini CRT model
  TC051 — 5 CRT model
  TC052 — 4.1 mini CRT model
...
Note           : All test cases are for Temporary Chat sessions — messages must NOT
                 ...
                 Document Review queries include grammatical/semantical errors to verify
                 the model identifies and shows proper response.
"""
...
async def _run_tc_flow(
    ...
    """Execute all 12 steps for Temporary Chat Initialization with Document Review type.

    Key differences from TC007-TC009 (regular Document Review):
    ...
    - Step 6     : Select model and Document Review type in Chat Configuration.
    - Step 7     : Verify empty query behavior — Send button remains disabled.
    - Step 8     : Submit query with grammatical/semantical errors; verify response
                   displays the errors and shows proper response with copy + regenerate.
    - Step 9     : Submit long query (>3000 words with grammatical/semantical errors).
    ...
    await _run_step(
        6,
        f"Go to Chat Configuration, select Model = {model_name} and Chat Type = Document Review",
    ...
    await _run_step(
        8,
        "Verify Chat Initialization — enter a CareSource-related query with special characters, "
        f"grammatical and semantical errors (e.g. '{query}') and click Send; "
        "verify response displays errors/corrections and shows copy icon and Regenerate Response button",
        _verify_temporary_chat_response(page, chat_page, query),
        tc_name, page,
    )
    await _run_step(
        9,
        "Verify long query (>3000 words with grammatical/semantical errors) — "
...
async def test_temporary_chat_initialization_document_review(
    docuchat_context,
    model_name: str,
    tc_name: str,
    tc_key: str,
):
    """Validate Temporary Chat Initialization with Document Review type across all model variants.

    Parametrize IDs: TC050 (5-mini CRT), TC051 (5 CRT), TC052 (4.1 mini CRT).
    To run a single variant: pytest -k TC050
