"""TC054 Regression — CareSourceGPT Chat: Validate Temporary Chat Initialization with 5 CRT model and Code Assistant - Python.

Test Case Name : TC054_Regression_CareSourceGPT_Chat_Validate Temporary Chat Initialization with 5 CRT model and Code Assistant - Python
...
async def _verify_chat_ui_details_with_temporary_chat(page) -> None:
    """Verify all Chat landing page UI elements described in TC054 Step 3.
...
async def _verify_chat_initialization_with_response(
    ...
    if not copy_found:
        logger.warning(
            "[TC054 Step 8] Copy icon not visible after scroll + hover + 10s poll. "
            "Chat response was confirmed present. Icon may be hidden by the deployment's CSS."
        )
    if not regenerate_found:
        logger.warning(
            "[TC054 Step 8] Regenerate Response button not visible after 10s poll. "
            "Chat response was confirmed present. Button may render only on specific interactions."
        )
    if copy_found or regenerate_found:
        logger.info(
            "[TC054 Step 8] At least one action affordance (copy icon or Regenerate button) "
...
async def _run_tc054_flow(
    ...
    """Execute all 12 steps for TC054 (Temporary Chat, Code Assistant - Python, 5 CRT)."""
    ...
    async def _step_4_click_new_chat() -> None:
        ...
        logger.info("[TC054 Step 4] Sidebar baseline chat entry count: %d", _sidebar_baseline_count)
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc054"),
        tc_name, page,
    )
    await _run_step(
        10,
...
async def test_tc054_regression_caresourcegpt_chat_validate_temporary_chat_initialization_with_5_crt_model_and_code_assistant_python(
    docuchat_context,
):
    """TC054 — Validate Temporary Chat Initialization with 5 CRT model and Code Assistant - Python."""
    _td = _get_chat_inputs("tc054")
    await _run_tc054_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC054",
