"""TC016 Regression — DocuChat Chat: Validate user level saved prompt with 5-mini CRT model and Basic Chat type.

Test Case Name : TC016_Regression_DocuChat_Chat_Validate user level saved prompt with
                 5-mini CRT model and Basic Chat type
Description    : Verify DocuChat initializes Chat successfully using Saved Prompts from Prompt
                 Library and user can start interaction, handles very long queries gracefully
                 without any error, handles chat Edit functionality from the Chat selected in
                 Chat history without any errors.
Pre-condition  : User has valid DocuChat access.
                 Application should have Prompts created by user (specific to Chat type selected
                 and support long queries of 3000 words) in Prompt Library and can be selected in
...
_TC016_NAME = (
    "TC016_Regression_DocuChat_Chat_Validate user level saved prompt with "
    "5-mini CRT model and Basic Chat type"
)
...
async def _run_tc_flow(
    ...
    """Execute all 11 steps for TC016.

    Step 5  : Open Saved Prompts section, select a saved prompt, verify it populates
              the 'How can I help?' text area.
...
async def test_tc016_regression_docuchat_chat_validate_user_level_saved_prompt_with_5mini_crt_model_and_basic_chat_type(
    docuchat_context,
):
    """TC016 — Validate user level saved prompt with 5-mini CRT model and Basic Chat type."""
    _td = _get_chat_inputs("tc016")
    await _run_tc_flow(
        docuchat_context,
        query=_td.query,
        tc_name=_TC016_NAME,
