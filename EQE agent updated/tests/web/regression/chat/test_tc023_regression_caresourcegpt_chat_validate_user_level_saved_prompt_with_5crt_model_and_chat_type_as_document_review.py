"""TC023 Regression — CareSourceGPT Chat: Validate user level saved prompt with 5 CRT model and Chat type as Document Review.

Test Case Name : TC023_Regression_CareSourceGPT_Chat_Validate user level saved prompt with 5 CRT model and Chat type as Document Review
...
_TC023_EDITED_CHAT_TITLE = "Edited Chat Title"
...
async def _verify_chat_ui_details_caresourcegpt(page) -> None:
    """Verify all Chat landing page UI elements described in TC023 Step 3."""
    checks = [
        (
            "New Chat button",
            [
...
async def _verify_long_query_with_document_review_saved_prompt(
    ...
    if selected_text and len(selected_text.split()) >= 100:
        ...
        await _verify_long_query_response(chat_page, tc_key="tc023")
...
async def _initiate_chat_with_updated_chat(page, chat_page: ChatPage) -> None:
    ...
    Searches the sidebar for a chat whose title contains _TC023_EDITED_CHAT_TITLE.
    ...
    for index in range(button_count):
        try:
            ...
            if _TC023_EDITED_CHAT_TITLE.lower() in text.lower():
...
async def _run_tc023_flow(
    ...
    """Execute all 11 steps for TC023 (Document Review, 5 CRT, Saved Prompts)."""
...
async def test_tc023_regression_caresourcegpt_chat_validate_user_level_saved_prompt_with_5crt_model_and_chat_type_as_document_review(
    docuchat_context,
):
    """TC023 — Validate user level saved prompt with 5 CRT model and Chat type as Document Review."""
    _td = _get_chat_inputs("tc023")
    await _run_tc023_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC023",
