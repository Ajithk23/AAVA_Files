"""TC027 Regression — CareSourceGPT Chat: Validate user level saved prompt with 4.1-mini CRT model and Chat type as Code Assistant - Python.

Test Case Name : TC027_Regression_CareSourceGPT_Chat_Validate user level saved prompt with 4.1-mini CRT model and Chat type as Code Assistant - Python
...
_TC027_EDITED_CHAT_TITLE = "Edited Chat Title"
...
async def _verify_chat_ui_details_caresourcegpt(page) -> None:
    """Verify all Chat landing page UI elements described in TC027 Step 3."""
    checks = [
        (
            "New Chat button",
            [
...
async def _verify_long_query_with_document_review_saved_prompt(
    ...
    if selected_text and len(selected_text.split()) >= 100:
        ...
        await _verify_long_query_response(chat_page, tc_key="tc027")
...
async def _initiate_chat_with_updated_chat(page, chat_page: ChatPage) -> None:
    ...
    Searches the sidebar for a chat whose title contains _TC027_EDITED_CHAT_TITLE.
    ...
    for index in range(button_count):
        try:
            ...
            if _TC027_EDITED_CHAT_TITLE.lower() in text.lower():
...
async def _run_tc027_flow(
    ...
    """Execute all 11 steps for TC027 (Code Assistant - Python, 4.1-mini CRT, Saved Prompts)."""
...
async def test_tc027_regression_caresourcegpt_chat_validate_user_level_saved_prompt_with_41mini_crt_model_and_chat_type_as_code_assistant_python(
    docuchat_context,
):
    """TC027 — Validate user level saved prompt with 4.1-mini CRT model and Chat type as Code Assistant - Python."""
    _td = _get_chat_inputs("tc027")
    await _run_tc027_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC027",
