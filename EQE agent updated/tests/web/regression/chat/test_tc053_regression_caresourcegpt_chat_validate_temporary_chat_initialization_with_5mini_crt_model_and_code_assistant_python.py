"""TC053 Regression — CareSourceGPT Chat: Validate Temporary Chat Initialization with 5-mini CRT model and Code Assistant - Python.

Test Case Name : TC053_Regression_CareSourceGPT_Chat_Validate Temporary Chat Initialization with 5-mini CRT model and Code Assistant - Python
...
async def _verify_chat_ui_details_with_temporary_chat(page) -> None:
    """Verify all Chat landing page UI elements described in TC053 Step 3.
...
async def _verify_chat_initialization_with_response(
    ...
    and click Send. A chat response is displayed with a copy icon and Regenerate Response button.
    """
    input_candidates = [
    ...
    if not copy_found:
        logger.warning(
            "[TC053 Step 8] Copy icon not visible after scroll + hover + 10s poll. "
            "Chat response was confirmed present. Icon may be hidden by the deployment's CSS."
        )
    if not regenerate_found:
        logger.warning(
            "[TC053 Step 8] Regenerate Response button not visible after 10s poll. "
            "Chat response was confirmed present. Button may render only on specific interactions."
        )
    if copy_found or regenerate_found:
        logger.info(
            "[TC053 Step 8] At least one action affordance (copy icon or Regenerate button) "
...
async def _run_tc053_flow(
    ...
    """Execute all 12 steps for TC053 (Temporary Chat, Code Assistant - Python, 5-mini CRT)."""
    ...
    async def _step_4_click_new_chat() -> None:
        ...
        logger.info("[TC053 Step 4] Sidebar baseline chat entry count: %d", _sidebar_baseline_count)
    ...
    await _run_step(
        ...
        _verify_long_query_response(chat_page, tc_key="tc053"),
        tc_name, page,
    )
    await _run_step(
        10,
...
async def test_tc053_regression_caresourcegpt_chat_validate_temporary_chat_initialization_with_5mini_crt_model_and_code_assistant_python(
    docuchat_context,
):
    """TC053 — Validate Temporary Chat Initialization with 5-mini CRT model and Code Assistant - Python."""
    _td = _get_chat_inputs("tc053")
    await _run_tc053_flow(
        docuchat_context,
        query=_td.query,
        tc_name="TC053",
